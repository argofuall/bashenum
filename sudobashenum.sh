...
# =============================================================================
# System Security Audit Script (Sudo Version)
# =============================================================================
...
# Verify sudo access at the beginning
verify_sudo() {
    if sudo -v; then
...
if command_exists systemctl; then
    execute_cmd systemctl list-units --type=service --state=running
...
WW_FILES_COUNT=$(execute_cmd find / -type f -perm -o+w 2>/dev/null | wc -l)
echo "Total World-Writable Files: $WW_FILES_COUNT"
execute_cmd find / -type f -perm -o+w 2>/dev/null | head -n 20
echo

# World-Writable Directories
print_subheader "World-Writable Directories"
WW_DIRS_COUNT=$(execute_cmd find / -type d -perm -o+w 2>/dev/null | wc -l)
echo "Total World-Writable Directories: $WW_DIRS_COUNT"
execute_cmd find / -type d -perm -o+w 2>/dev/null | head -n 20
echo

# Sensitive Files Detection
print_subheader "Sensitive Files Detection"
SENSITIVE_FILES=$(execute_cmd find /home /root /etc -type f \( -iname "*.env" -o -iname "id_rsa" -o -iname "*.pem" -o -iname "*.key" \) 2>/dev/null)
...
if command_exists systemctl; then
    execute_cmd systemctl status auditd
...
for user in $(cut -f1 -d: /etc/passwd); do
    ...
    execute_cmd crontab -u "$user" -l 2>/dev/null || echo "No cron jobs for user: $user"
    echo
done
echo
...
if command_exists dpkg; then
    ...
    PENDING_UPDATES=$(execute_cmd apt-get -s upgrade | grep "^Inst" | grep -E 'security|updates') || PENDING_UPDATES=""
    ...
    PENDING_UPDATES=$(execute_cmd yum check-update) || PENDING_UPDATES=""
    if [ -n "$PENDING_UPDATES" ]; then
        echo "Pending Security Updates:"
        echo "$PENDING_UPDATES"
    else
